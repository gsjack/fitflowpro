...
tap.test('Integration Test: Analytics and Progression Tracking (T076)', async (t) => {
    ...
    t.test('should track 1RM progression over 4 weeks (120kg → 130kg)', async (t) => {
      ...
      ORDER BY w.date
    `);
        const progression = progressionQuery.all(userId, exerciseId);
    ...
    t.test('should track volume trends with MEV/MAV/MRV landmarks', async (t) => {
        ...
        const weeks = [
            { week: 1, totalSets: 14, phase: 'MAV' },
            { week: 2, totalSets: 16, phase: 'approaching_MRV' },
            { week: 3, totalSets: 18, phase: 'MRV' },
            { week: 4, totalSets: 10, phase: 'deload' },
        ];
        const insertWorkout = db.prepare('INSERT INTO workouts (user_id, date, status) VALUES (?, ?, ?)');
        const insertSet = db.prepare('INSERT INTO sets (workout_id, exercise_id, set_number, weight_kg, reps, rir) VALUES (?, ?, ?, ?, ?, ?)');
      ...
      ORDER BY week
    `);
        const volumeTrends = volumeQuery.all(userId, chestExerciseId);
        ...
        volumeTrends.forEach((trend, index) => {
            ...
            if (index === 0)
                t.equal(zone, 'MAV_to_MRV', 'Week 1 should be in MAV-MRV zone');
            if (index === 1)
                t.equal(zone, 'MAV_to_MRV', 'Week 2 should be in MAV-MRV zone');
            if (index === 2)
                t.equal(zone, 'MAV_to_MRV', 'Week 3 should be in MAV-MRV zone');
            if (index === 3)
                t.equal(zone, 'MEV_to_MAV', 'Week 4 (deload) should be in MEV-MAV zone');
        });
    });
    t.test('should calculate adherence and consistency metrics', async (t) => {
        ...
        for (let week = 1; week <= 4; week++) {
            for (let day = 0; day < 6; day++) {
                ...
                const duration = 50 + Math.floor(Math.random() * 10);
    ...
    `);
        const adherenceResult = adherenceQuery.get(userId);
    ...
    t.test('should verify deload compliance', async (t) => {
        ...
        const week3Id = week3Result.lastInsertRowid;
        for (let i = 1; i <= 18; i++) {
        ...
        const week4Id = week4Result.lastInsertRowid;
        for (let i = 1; i <= 10; i++) {
      ...
      ORDER BY w.date
    `);
        const volumes = volumeQuery.all(week3Id, week4Id);
    ...
    `);
        const volumeByMuscle = volumeByMuscleQuery.all(userId);
...
//# sourceMappingURL=analytics-progression.test.js.map