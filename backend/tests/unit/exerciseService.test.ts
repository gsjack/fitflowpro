 ...
 *
 * Tests exercise filtering and retrieval logic using the real database
 * Note: These tests run against the seeded production database (70 exercises)
 * Target coverage: ≥ 80%
 */
...
tap.test('Exercise Service Unit Tests', async (t) => {
  await t.test('getExercises()', async (t) => {
    ...
    await t.test('should filter by muscle_group (secondary)', async (t) => {
      ...
      t.ok(
        exercises.every(
          (ex) =>
            ex.primary_muscle_group === 'triceps' ||
            ex.secondary_muscle_groups.includes('triceps')
        ),
        'All exercises target triceps'
      );
    });
    ...
    await t.test('should return empty array for no matches', async (t) => {
      ...
      // This combination likely doesn't exist, but we can't guarantee it
      // Just test that filtering works without errors
      t.ok(Array.isArray(exercises), 'Returns array');
    });

    await t.test('should throw error for invalid muscle_group', async (t) => {
      t.throws(
        () => getExercises({ muscle_group: 'invalid' }),
        /Invalid muscle_group/,
        'Throws error for invalid muscle group'
      );
    });
  });
  ...
  await t.test('getExerciseById()', async (t) => {
    ...
    await t.test('should parse secondary_muscle_groups correctly', async (t) => {
      // Find an isolation exercise (movement_pattern = 'isolation')
