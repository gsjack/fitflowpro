/**
 * Program Exercise Service Unit Tests
 *
 * Tests CRUD operations, exercise swapping, reordering, and volume validation.
 * Uses the actual database service (tests run against test data).
 */
...
import {
  ...
  swapExercise,
  reorderExercises,
} from '../../src/services/programExerciseService.js';
...
tap.test('Program Exercise Service Unit Tests', async (t) => {
  ...
  let chestExerciseId: number;
  let altChestExerciseId: number;
  let backExerciseId: number;
  ...
  await t.before(async () => {
    ...
    const userResult = userStmt.run(
      `test-pe-service-${Date.now()}@example.com`,
      'hash',
      28,
      75,
      'intermediate',
    ...
    const chestResult = exerciseStmt.run(
      'Barbell Bench Press Unit Test',
      JSON.stringify(['chest', 'front_delts', 'triceps']),
      'barbell',
      'intermediate',
      4,
      '6-8',
    ...
    chestExerciseId = chestResult.lastInsertRowid as number;

    const altChestResult = exerciseStmt.run(
      'Dumbbell Bench Press Unit Test',
      JSON.stringify(['chest', 'front_delts', 'triceps']),
      'dumbbell',
      'intermediate',
      4,
    ...
    altChestExerciseId = altChestResult.lastInsertRowid as number;

    const backResult = exerciseStmt.run(
      'Barbell Row Unit Test',
      JSON.stringify(['back_lats', 'back_traps', 'biceps']),
      'barbell',
      'intermediate',
      4,
      '6-8',
    ...
    backExerciseId = backResult.lastInsertRowid as number;
    ...
    programId = programResult.lastInsertRowid as number;

    // Create test program day
    ...
    programDayId = programDayResult.lastInsertRowid as number;
  });

  // Test: getProgramExercises
  ...
  await t.test('createProgramExercise', async (t) => {
    await t.test('should create program exercise successfully', async (t) => {
      ...
      t.ok(
        result.volume_warning === null || typeof result.volume_warning === 'string',
        'Returns volume_warning'
      );
    });
    ...
    await t.test('should return volume warning when exceeding MRV', async (t) => {
      // Create a fresh program day
      const newProgramDayStmt = db.prepare(`
        INSERT INTO program_days (program_id, day_of_week, day_name, day_type)
        VALUES (?, ?, ?, ?)
      `);
      const newProgramDayResult = newProgramDayStmt.run(programId, 2, 'MRV Test Day', 'strength');
      const mrvTestDayId = newProgramDayResult.lastInsertRowid as number;

      // Add many exercises to push volume over MRV (chest MRV = 22)
      for (let i = 0; i < 6; i++) {
        createProgramExercise({
    ...
    await t.test('should throw error for non-existent program_day_id', async (t) => {
      t.throws(
        ...
        /not found/i,
        'Throws error for non-existent program_day_id'
      );
    });
    ...
    await t.test('should throw error for non-existent exercise_id', async (t) => {
      t.throws(
        ...
        /not found/i,
        'Throws error for non-existent exercise_id'
      );
    });
  });

  // Test: updateProgramExercise
  await t.test('updateProgramExercise', async (t) => {
    let programExerciseId: number;
    let updateTestDayId: number;

    await t.before(async () => {
      // Create a fresh program day and exercise for update tests
      const newProgramDayStmt = db.prepare(`
        INSERT INTO program_days (program_id, day_of_week, day_name, day_type)
        VALUES (?, ?, ?, ?)
      `);
      const newProgramDayResult = newProgramDayStmt.run(programId, 3, 'Update Test Day', 'strength');
      updateTestDayId = newProgramDayResult.lastInsertRowid as number;
    ...
    await t.test('should throw error for non-existent program_exercise_id', async (t) => {
      t.throws(
        ...
        /not found/i,
        'Throws error for non-existent program_exercise_id'
      );
    });
  });

  // Test: deleteProgramExercise
  await t.test('deleteProgramExercise', async (t) => {
    await t.test('should delete program exercise', async (t) => {
      // Create a fresh program day and exercise for delete test
      const newProgramDayStmt = db.prepare(`
        INSERT INTO program_days (program_id, day_of_week, day_name, day_type)
        VALUES (?, ?, ?, ?)
      `);
      const newProgramDayResult = newProgramDayStmt.run(programId, 4, 'Delete Test Day', 'strength');
      const deleteTestDayId = newProgramDayResult.lastInsertRowid as number;
    ...
    await t.test('should return volume warning when dropping below MEV', async (t) => {
      // Create a NEW isolated program for MEV testing to avoid interference
      const mevProgramStmt = db.prepare(`
        INSERT INTO programs (user_id, name, mesocycle_week, mesocycle_phase, created_at)
        VALUES (?, ?, ?, ?, ?)
      `);
      const mevProgramResult = mevProgramStmt.run(userId, 'MEV Test Program', 1, 'mav', Date.now());
      const mevProgramId = mevProgramResult.lastInsertRowid as number;

      // Create a minimal program day with only MEV volume
      ...
      const mevTestDayId = minimalProgramDayResult.lastInsertRowid as number;
      ...
      // Now we have 9 chest sets total (5 + 4). Delete one exercise to drop to 5 sets (below MEV=8)
      ...
      // Clean up isolated test program
  ...
  await t.test('swapExercise', async (t) => {
    let programExerciseId: number;
    let swapTestDayId: number;

    await t.before(async () => {
      // Create a fresh program day for swap tests
      const swapProgramDayStmt = db.prepare(`
        INSERT INTO program_days (program_id, day_of_week, day_name, day_type)
        VALUES (?, ?, ?, ?)
      `);
      const swapProgramDayResult = swapProgramDayStmt.run(programId, 6, 'Swap Test Day', 'strength');
      swapTestDayId = swapProgramDayResult.lastInsertRowid as number;
    ...
    await t.test('should preserve order_index after swap', async (t) => {
      ...
      swapExercise(programExerciseId, chestExerciseId); // Swap back
    ...
    await t.test('should throw error for incompatible muscle groups', async (t) => {
      t.throws(
        ...
        /incompatible/i,
        'Throws error for incompatible exercise'
      );
    });
    ...
    await t.test('should throw error for non-existent new_exercise_id', async (t) => {
      t.throws(
        ...
        /not found/i,
        'Throws error for non-existent new_exercise_id'
      );
    });
  });

  // Test: reorderExercises
  await t.test('reorderExercises', async (t) => {
    let reorderProgramDayId: number;
    let ex1Id: number;
    let ex2Id: number;
    let ex3Id: number;

    await t.before(async () => {
      // Create a fresh program day with multiple exercises
      ...
      reorderProgramDayId = reorderProgramDayResult.lastInsertRowid as number;
  ...
  // Test: Volume calculation with multi-muscle exercises
  await t.test('Volume calculation with multi-muscle exercises', async (t) => {
    await t.test('should count full sets for each targeted muscle group', async (t) => {
      // Create a fresh program day (day_of_week must be 1-7)
      ...
      const volumeTestProgramDayId = volumeTestProgramDayResult.lastInsertRowid as number;

      // Add Barbell Bench Press (targets chest, front_delts, triceps) - 4 sets
      createProgramExercise({
      ...
      // Each of chest, front_delts, triceps should have +4 sets
      // This is validated by the volume warning logic in the service

      // Add another chest exercise to verify accumulation
      createProgramExercise({
        program_day_id: volumeTestProgramDayId,
        exercise_id: altChestExerciseId,
        target_sets: 4,
      ...
      // Now chest should have 8 sets total (4 + 4)
      // This test verifies the accumulation logic works correctly
