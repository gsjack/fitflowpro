...
tap.test('Program Service - advancePhase()', async (t) => {
    ...
    await t.test('MEV → MAV: Automatic progression with +20% volume', async (t) => {
        const initialProgram = db.prepare('SELECT * FROM programs WHERE id = ?').get(programId);
    ...
    `).all(programId);
        ...
        const updatedProgram = db.prepare('SELECT * FROM programs WHERE id = ?').get(programId);
    ...
    await t.test('Error: Invalid target_phase', async (t) => {
        t.throws(() => advancePhase(programId, true, 'invalid_phase'), /Invalid target_phase/, 'Throws error for invalid phase');
    });
    await t.test('Error: Manual=true but target_phase missing', async (t) => {
        t.throws(() => advancePhase(programId, true), /target_phase is required/, 'Throws error when manual=true but target_phase is missing');
    });
    await t.test('Error: Program not found', async (t) => {
        t.throws(() => advancePhase(99999, false), /Program with ID 99999 not found/, 'Throws error for non-existent program');
    });
    ...
    `).run(userId, 'Test Volume Program', 1, 'mev', now);
        const testProgramId = testProgramResult.lastInsertRowid;
    ...
    `).run(testProgramId, 1, 'Test Day', 'strength');
        const testDayId = testDayResult.lastInsertRowid;
        db.prepare(`
...
//# sourceMappingURL=programService.test.js.map