...
tap.test('Program Management Endpoints Contract Tests', async (t) => {
    ...
    await t.test('GET /api/programs', async (t) => {
        await t.test('should return active program with full structure (200)', async (t) => {
            ...
            t.ok(['mev', 'mav', 'mrv', 'deload'].includes(body.mesocycle_phase), 'Program has valid mesocycle_phase');
            t.type(body.mesocycle_week, 'number', 'Program has mesocycle_week');
            t.ok(body.mesocycle_week >= 1 && body.mesocycle_week <= 8, 'mesocycle_week is between 1 and 8');
            ...
            t.ok(programDay.day_of_week >= 1 && programDay.day_of_week <= 7, 'day_of_week is between 1 and 7');
            ...
            if (programDay.exercises.length > 0) {
                ...
                t.ok(programExercise.target_sets >= 1 && programExercise.target_sets <= 10, 'target_sets is between 1 and 10');
                t.type(programExercise.target_rep_range, 'string', 'ProgramExercise has target_rep_range');
                t.match(programExercise.target_rep_range, /^\d+-\d+$/, 'target_rep_range follows format "6-8"');
                t.type(programExercise.target_rir, 'number', 'ProgramExercise has target_rir');
                t.ok(programExercise.target_rir >= 0 && programExercise.target_rir <= 4, 'target_rir is between 0 and 4');
    ...
    await t.test('PATCH /api/programs/:id/advance-phase', async (t) => {
        ...
        await t.test('should advance phase automatically (MEV→MAV) (200)', async (t) => {
            ...
            t.ok(['mev', 'mav', 'mrv', 'deload'].includes(body.previous_phase), 'previous_phase is valid');
            t.type(body.new_phase, 'string', 'Response has new_phase');
            t.ok(['mev', 'mav', 'mrv', 'deload'].includes(body.new_phase), 'new_phase is valid');
            ...
            if (body.previous_phase === 'mev') {
                ...
                t.ok(body.volume_multiplier >= 1.15 && body.volume_multiplier <= 1.25, 'MEV→MAV volume multiplier is ~1.2');
            }
        });
    ...
    await t.test('GET /api/programs/:id/volume', async (t) => {
        ...
        await t.test('should generate warnings for suboptimal volume', async (t) => {
            ...
            const body = response.json();
            const belowMevGroups = body.muscle_groups.filter((mg) => mg.zone === 'below_mev');
            const aboveMrvGroups = body.muscle_groups.filter((mg) => mg.zone === 'above_mrv');
            const belowMevWarnings = body.warnings.filter((w) => w.issue === 'below_mev');
            const aboveMrvWarnings = body.warnings.filter((w) => w.issue === 'above_mrv');
            t.equal(belowMevWarnings.length, belowMevGroups.length, 'Warning count matches below_mev muscle groups');
            t.equal(aboveMrvWarnings.length, aboveMrvGroups.length, 'Warning count matches above_mrv muscle groups');
        });
        await t.test('should return 401 without authentication', async (t) => {
            const response = await app.inject({
                method: 'GET',
