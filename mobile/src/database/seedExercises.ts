/**
 * Mobile Exercise Seeding Script
 *
 * Seeds mobile SQLite database with Renaissance Periodization exercise library.
 * Must be run once on app initialization.
 *
 * Usage:
 * - Called from initializeDatabase() if exercises table is empty
 * - Inserts 70+ exercises covering all muscle groups
 */

import { runAsync } from './db';

/**
 * Seed exercise data into mobile database
 * Returns number of exercises inserted
 */
export async function seedExercises(): Promise<number> {
  console.log('[SeedExercises] Starting exercise seeding...');
  ...
  const exercises = [
    ...
    [
      'Barbell Row',
      '["lats","mid_back","rear_delts"]',
      'barbell',
      'intermediate',
      4,
      '6-8',
      2,
      'Compound back builder',
    ],
    [
      'Dumbbell Row',
      '["lats","mid_back","rear_delts"]',
    ...
    [
      ...
      '["hamstrings","glutes","lower_back","traps"]',
      'barbell',
      'advanced',
      4,
      '5-6',
    ...
    [
      'Trap Bar Deadlift',
      '["quads","hamstrings","glutes","traps"]',
      'barbell',
      'intermediate',
      4,
      '6-8',
      2,
      'Safer deadlift variant',
    ],
  ];
  ...
  for (const exercise of exercises) {
    try {
      ...
      console.error(`[SeedExercises] Failed to insert exercise: ${exercise[0]}`, error);
    }
  }

  console.log(`[SeedExercises] Seeded ${insertedCount} exercises`);
 ...
 */
export async function areExercisesSeeded(): Promise<boolean> {
  const { getAllAsync } = await import('./db');
  const result = await getAllAsync<{ count: number }>(
