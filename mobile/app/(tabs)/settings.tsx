/**
 * SettingsScreen (T066) - Migrated to Expo Router
 *
...
import { getToken, clearToken } from '../../src/services/api/authApi';
import {
...
export default function SettingsScreen() {
  ...
  const confirmLogout = async () => {
    ...
    try {
      // Clear token
      await clearToken();
      console.log('[SettingsScreen] Token cleared, navigating to auth');

      // Navigate to auth screen
      router.replace('/(auth)/login');
    } catch (error) {
      console.error('[SettingsScreen] Logout failed:', error);
      // Still navigate to auth as fallback
      router.replace('/(auth)/login');
    }
  };
  ...
  const handleDeleteAccountSuccess = async () => {
    // Navigate to auth screen (token already cleared by DeleteAccountModal)
    await clearToken();
    router.replace('/(auth)/login');
  };
